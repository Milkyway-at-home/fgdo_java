...
public class Team extends DatabaseEntry implements ExpAvgItem {

	public Team(int id) throws DatabaseRetrieveException {
		this.id = id;

		Connection connection = BoincDatabase.getConnection();
		try {
			Statement statement = connection.createStatement();
			ResultSet rs = statement.executeQuery("SELECT expavg_credit, expavg_time FROM team WHERE id = " + id);

			if (!rs.first()) {
				throw new DatabaseRetrieveException("Could not lookup team: " + id + ", empty ResultSet.");
			}       

			this.expavg_credit = rs.getDouble(1);
			this.expavg_time = rs.getDouble(2);

			rs.close();
			statement.close();

		} catch (SQLException ex) {
			throw new DatabaseRetrieveException("Could not lookup team: " + id, ex);
		}
	}
	...
	public void commit() throws DatabaseCommitException {
		super.commit("UPDATE team", "WHERE id = " + getId());
	}
}
